...
import { POINTS_TYPES } from '../mocks/mocks-point';
...
function createTemplate(point, offers, destination, offersModel, destinationsModel) {
  ...
  const pointIconMarkup = POINTS_TYPES.map((type) => `
  ...
  type="text" name="event-destination" value="${destination.name}" list="destination-list-1"><datalist id="destination-list-1">${destinationsNamesMarkup}
  </datalist>
  </div>
  ...
  <input class="event__input event__input--time" id="event-end-time-1"type="text" name="event-end-time" value="${dateEnd}"></div>

  <div class="event__field-group event__field-group--price">
  <label class="event__label" for="event-price-1">
  <span class="visually-hidden">Price</span>€${point.price}
  </label>
